...
def test_dictionary_is_mutable():
    ...
    assert {'one':10, 'three': 3, 'two': 2} == word_to_digit
...
def test_valid_dictionary_keys():
    ...
    try:
        ...
        print te  #observe the error message. --- unhashable type: 'list'
        assert test_dict == {1: 1,'one': 'string'}
    ...
    try:
        ...
        print te
        assert test_dict == {} # do we reach here?
                                 # No, the key-value pair is inserted into the dictionary and the String value is
                                 # identified by the tuple(key) : (1,2)
    ...
    try:
        ...
        print te
        assert te  #do we reach here?
                                        #  Yes, tuples contains immutable objects,here key value is a tuple
                                        #   having list(mutable objects) so we get Error
...
- Dicitionaries are used to store the values by mapping them to keys
- A value in dictionary is searched through its key and not by its index.
- (Key,Value) pairs in dictionary might change their order depending on other pairs(through hashing)
"""
