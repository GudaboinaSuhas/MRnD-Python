...
def get_lower_to_upper_dict():
    ...
    """
    a=dict(zip(string.ascii_lowercase,string.ascii_uppercase))
    return a
...
def get_encoding_dict():
    ...
    b=dict(zip(string.ascii_uppercase,range(1,27)))
...
def get_consonents(input):
    ...
    No control flow plz.
    """
    s=set(input.lower())-set(VOWEL_SET)
    return s
...
def capitalize_names(input):
    ...
    """
    a = map(lambda x: x.capitalize(),input)
    return list(a)
