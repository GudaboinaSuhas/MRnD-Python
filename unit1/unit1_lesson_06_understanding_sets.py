...
def test_set_type():
    ...
    assert set == type(test_set)

def test_set_empty():
    empty_set_wrong = {}  #curly braces are used for both sets and dicts, so how do you disambiguate?
    assert False == isinstance(empty_set_wrong, set)   #There is no way of determining if it is a set or dict unless
                                                       #it contains some string or (k,v)pair or we have to create a set
...
- set are same as dictionaries but doesnt exist in pairs
- set provides membership testing and removing of duplicates
- it can perform some math operations such as union,intersection and difference
"""
